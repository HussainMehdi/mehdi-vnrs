...
describe("RegisterVanityDomain", function () {
    ...
    it("only first one to commit will be able to register", async () => {
        ...
        const offchainHash = vreg.getVanityRecordHash(defaultRecord);
        const offchainHashBob = vreg.getVanityRecordHash({ ...defaultRecord, user: bob });
        const VNRS = (await ethers.getContractFactory("VNRS")).attach(vnrsAddr);
        ...
        // for bob
        const bobSolidityStruct = vreg.vanityRecordToSolidity({ ...defaultRecord, user: bob });
        ...
        const bobRegisterVanityDomainResp = await (await VNRS.connect(bobSigner).registerVanityDomain(bobTD)).wait();
    ...
    it("only first one to commit will be able to register even if bob registers first", async () => {
        ...
        expect(bobRegisterVanityDomainResp.events[0].event).to.be.equal('LogRegisteredVanityRecord');
    ...
    it("bob can only register same name again if alice registration grace time is expired", async () => {
        ...
        defaultRecord.user = alice;
        const offchainHash = vreg.getVanityRecordHash(defaultRecord);
        let offchainHashBob = vreg.getVanityRecordHash({ ...defaultRecord, user: bob });
        const VNRS = (await ethers.getContractFactory("VNRS")).attach(vnrsAddr);
        ...
        // for bob
        let bobSolidityStruct = vreg.vanityRecordToSolidity({ ...defaultRecord, user: bob });
        let bobTypedSig = await vreg.getSignedVanityStruct(bobSolidityStruct);
        let bobTD = vreg.toSolidityByteVanityRecord(bobTypedSig);
        let bobRegisterVanityDomainResp = await (await VNRS.connect(bobSigner).registerVanityDomain(bobTD)).wait();
        ...
        offchainHashBob = vreg.getVanityRecordHash({ ...defaultRecord, user: bob, salt: new BigNumber(3232) });
        await (await VNRS.commitRegistration(offchainHashBob)).wait();
        // for bob again
        bobSolidityStruct = vreg.vanityRecordToSolidity({ ...defaultRecord, user: bob, salt: new BigNumber(3232) });
        bobTypedSig = await vreg.getSignedVanityStruct(bobSolidityStruct);
        bobTD = vreg.toSolidityByteVanityRecord(bobTypedSig);
        bobRegisterVanityDomainResp = await (await VNRS.connect(bobSigner).registerVanityDomain(bobTD)).wait();
        expect(bobRegisterVanityDomainResp.events[0].event).to.be.equal('LogRegisteredVanityRecord');
    })
