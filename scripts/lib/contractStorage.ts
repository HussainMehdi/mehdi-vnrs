import { DeployedContracts, FriendlyName, friendlyNames } from "./IDeployedContracts";
...
if (existsSync('deployedContracts.json')) {
    deployedContracts = JSON.parse(readFileSync('deployedContracts.json').toString()) as DeployedContracts;
} else {
    writeFileSync('deployedContracts.json', JSON.stringify(deployedContracts));
}

export function storeContract(contract: Contract, name: string, networkChainId: string, friendlyName: FriendlyName = 'HARDHAT') {
...
export function getContractStorage(name: string, networkChainId: string, friendlyName: FriendlyName = "HARDHAT") {
    return deployedContracts[networkChainId]
        ...
        deployedContracts[networkChainId][friendlyName][name] : {} as any;
}

export async function saveDeployedContracts() {
    await writeFileSync('deployedContracts.json', JSON.stringify(deployedContracts));
}


function initFriendlyNames(contracts: DeployedContracts, chainId: string, friendlyName: readonly string[]) {
